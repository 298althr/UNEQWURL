 ...
 * Called after upload to automatically analyze before user can mix.
 *
 * This runs the same pipeline as scripts/run-analysis-pipeline.ts but for a
 * single track, and can be called from API routes.
 */
...
import { generateGenreBenchmark, type GenreHint } from "./genre-benchmark";
...
/**
 * Run full analysis on a single track and store results in DB.
...
export async function autoAnalyzeTrack(track: TrackInfo): Promise<boolean> {
  ...
  try {
    ...
    // Step 2: libsonare analysis
    await ensureInit();
    const analysis = await analyzeWithLibsonare(decoded);

    // Step 3: librosa features
    const features = extractFeaturesWithLibrosa(url, 120);

    // Step 4: Genre-aware benchmark (with upload_type hint if available)
    let genreHint = hint;
    ...
    // Step 4b: Look up reference track for genre comparison (Phase 2.5)
    // First detect genre to know which reference to fetch
    const { detectGenre } = await import("./genre-benchmark");
    ...
    // Update user_uploads with genre/bpm/key/duration + analysis_ready
    if (source === "upload") {
      await query(
...
/**
 * Check if a track has been analyzed and is ready for mixing.
 */
export async function isTrackAnalyzed(trackId: string, source: "song" | "upload"): Promise<boolean> {
  const { rows } = await query<{ exists: boolean }>(
    `SELECT EXISTS(SELECT 1 FROM audio_analysis WHERE track_id = $1 AND track_source = $2) as exists`,
    [trackId, source]
  );
