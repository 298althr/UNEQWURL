...
async function ensureBucketId(): Promise<string> {
  ...
  // Try to get bucketId from env var
  ...
  // Fallback: listBuckets (requires master key or listBuckets capability)
...
export function getDownloadUrl(fileName: string): string {
  // Friendly URL — file is public if bucket is public.
  // B2 friendly URL format: https://f003.backblazeb2.com/file/{bucket}/{fileName}
  // fileName may contain / for folders; only encode individual path segments.
...
export async function getFileStream(fileName: string): Promise<ReadableStream<Uint8Array>> {
  await ensureAuth();
  const b2 = getB2();
  ...
  const stream = (res as unknown as { data: ReadableStream<Uint8Array> }).data;
  return stream;
}
